...
def pitch_shift_endpoint():
    try:
        n_steps = int(request.args.get('n_steps', 0))  # Number of steps for pitch shifting
...
def process_audio():
    action = request.args.get('action')  # Get the action parameter from the query string
